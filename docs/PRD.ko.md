...
3. Signed demo history에서 add, inspect, revoke, outage, restart, audit, cleanup까지의 mutation path를 재현한다. RUN25 fast evidence는 이 경로를 포함하며 native expiry는 아직 open이다.
4. Native kernel expiry, host-ruleset invariance 및 five-minute 500-RPS/4-GB performance gate에 대한 독립 Linux evidence를 확보한다.
...
Fixed adapter contract는 공식 [`gpt-5.6-sol` model page](https://developers.openai.com/api/docs/models/gpt-5.6-sol), [model catalog](https://developers.openai.com/api/docs/models) 및 [Structured Outputs guide](https://developers.openai.com/api/docs/guides/structured-outputs)를 따른다. Model 또는 schema-version 변경 시 contract revalidation과 문서 갱신이 필요하다. Opt-in `cmd/openaismoke`는 로컬에서 disabled 및 missing-key fail-closed 동작을 증명했지만 live billable model result는 기록되지 않았다.
...
| FR-015 | MUST | 임시 rule은 자동 만료되고 안전한 read-only reconciliation과 별도의 manual revocation을 지원해야 한다. | Native timeout이 rule을 제거한다. 별도 서명된 `nft-inspect-v1`은 fixed read-back만 수행하고 mutation할 수 없다. 조기 소실은 failed/alerted 처리하고 자동 재추가하지 않으며 reapplication에는 새 validation/HIL이 필요하다. Deterministic `nft-revoke-v1`은 owned set에서만 삭제하고 독립 authorize, idempotent read-back 및 audit를 수행한다. |
...
| 가역성 | 승인된 임시 규칙이 지정 시각에 만료되고 감사 기록에 남는다. |
...
현재 evidence는 fresh/restart-noop·`33→24→33`·ACL·sqlc·repeated-content-digest identity·API-only validation-attempt projection·queued stale-analysis supersession을 포함하는 88-package backend build/test와 PostgreSQL 17.10 33-migration/72-table verifier의 final root rerun, contract vector, export, observability, security, nft namespace, threshold, performance smoke, 이전에 완료한 full reproducible-image/SBOM/vulnerability supply-chain gate, frontend Vitest file 39개/test 363개와 production-CSP Chromium gate 1/1의 final root evidence, root가 재검증한 demo-E2E helper 39/39·shell-contract 6/6을 포함한다. RUN25 fast Compose E2E(log SHA-256 `4702571db361b411449dadc789995348f0254f0a07a1a2aefda36a79b070b877`)는 mutation/outage/restart path를 다뤘고, 이후 macOS에서 실행한 `--fast --browser-qa-hold-seconds 900 --run-browser-qa`가 revoked phase의 pre-hash login-window에서 고정 61초를 기다린 뒤 login 재시도나 limit 변경 없이 active·revoked browser QA를 통과했다. Commit `d66c4b8a4842ad4226cb741e35331ba5b9068520`의 외부 clean clone도 `make check`를 통과했다. Hosted CI run `29696139988`은 implementation checkpoint `5ef870155bc59e6ac3c30279a7cd8be8d0249887`에서 frontend functional-browser 및 pinned Linux visual-baseline gate를 포함한 10개 shard를 모두 통과했다. 이 evidence는 release table을 충족하지 않는다. Default native-expiry run, native host-nft 및 4-GB/five-minute performance reference, release-level screenshot 및 live OpenAI call은 아직 pending이다. 상세 경계는 [구현 준비도](./IMPLEMENTATION_READINESS.ko.md)에 기록한다.
