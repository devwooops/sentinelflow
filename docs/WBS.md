...
The 2026-07-18 Gateway-first queue superseded the unexecuted 2026-07-17 log-first queue. The Gateway-first swarm has since produced an integrated implementation and local verification evidence in the shared workspace, but the tree is not yet a committed, clean-checkout baseline. Existing Syslog/parser Task IDs retain their meanings as P2 optional adapters; they do not enter this five-day queue or any release gate.
...
| Leaf 1 | `compose_browser_qa` | Local-only Compose browser runner and exact active/revoked action-state evidence | Latest macOS fast execution passed active/revoked browser QA. The revoked phase has a fixed 61-second pre-hash login-window wait with no login retry or limit change; release-level browser certification/screenshots remain pending |
| Leaf 2 | `clean_checkout_gate` | External temporary-snapshot clean-input preflight; no publication authority | Latest full run materialized 905 candidate source files, recorded manifest SHA-256 `2c395c3c5e3d28e908513e3304f5896ac7ae1eebe9a88dc80c543fe8baa73150`, and passed `make check`. It is source-only pre-commit evidence, not a clean-checkout/CI/Linux/release pass, and cannot complete a Tasklist item |
| Leaf 3 | `docs_sync_current_evidence` | `README.md` and PRD/ADR/TDD/Tasklist/WBS/readiness English/Korean pairs | Running; synchronizes the browser-QA result, exact executor dependency policy, and open release gates without changing task completion |
...
| --- | --- | --- |
| Contract and implementation waves | Integrated locally | Gateway-first contracts, backend/control plane, 33 migrations including provider-free stale queued-analysis supersession, staged signed-history activation, repeated-content-digest identity, and API-only projections, UI, dispatcher/executor, recovery, export, observability, and test harnesses exist; the untracked tree is not a clean-checkout baseline |
| Frontend contract certification | Final root rerun passed | Final root rerun reports 39 Vitest files/363 tests and deployment-CSP Chromium 1/1. The CSP-safe error decoder, exact header parser, and all-production-chunk dynamic-code-generation scan exist. A later macOS runner passed active/revoked browser QA, while the full frontend state/accessibility/visual/release evidence remains pending |
| Backend and safety certification | Final root reruns passed | The backend 88-package gate and PostgreSQL 17.10 33-migration/72-table verifier passed final root reruns, including fresh/restart-noop, `33→24→33`, ACL, sqlc, repeated-content identity, API projection, and stale-analysis supersession checks. Native Linux and release-duration conditions remain open |
| Compose E2E repair/certification | RUN25 fast plus later browser runner passed; release qualification open | RUN25, SHA-256 `4702571db361b411449dadc789995348f0254f0a07a1a2aefda36a79b070b877`, passed build/start, health/authority/private-origin isolation, exact coverage, all five scenarios, stable bindings, exact HIL add/inspect/revoke, digest-mismatch rejection, outage forwarding, restart/reconciliation, and cleanup. A later macOS `--run-browser-qa` execution passed active/revoked browser QA after its fixed 61-second pre-hash login-window wait. `--fast` revokes instead of waiting for native expiry, and macOS cannot certify host-ruleset invariance |
| Supply chain and final release | Prior full supply-chain evidence retained; final release open | The previously completed full rerun 3 passed reproducible images/SBOM/Trivy/runtime/lifecycle cleanup. Remaining goals are clean checkout/CI, default native-expiry/native host evidence, live OpenAI opt-in, 4 GB performance, final docs/screenshots, and release decision |

Tasklist checkboxes remain stricter than this wave ledger. Only `M0-001`, `M0-002`, `M0-009`, `M0-015`, `M0-017`, and `M0-019` are complete. `M0-005` still lacks a live OpenAI result; clean-checkout/CI, native host-nft, reusable worktree, Compose mutation, and five-minute 4 GB performance evidence remain incomplete. Therefore the current release classification is **Still implementing**.

The prior full supply-chain gate completed and cleaned up before Docker-mutating E2E work. The next default native-expiry run must capture a fresh global baseline and remain serialized from any other Docker-mutating gate.
