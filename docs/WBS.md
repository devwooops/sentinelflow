...
| --- | --- | --- |
| Contract and implementation waves | Published baseline plus local integration | Gateway-first contracts, backend/control plane, 33 migrations including provider-free stale queued-analysis supersession, staged signed-history activation, repeated-content-digest identity, and API-only projections, UI, dispatcher/executor, recovery, export, observability, and test harnesses exist; `d66c4b8a4842ad4226cb741e35331ba5b9068520` has clean-clone `make check` evidence while its post-repair CI rerun remains pending |
...
| Supply chain and final release | Prior full supply-chain and clean-clone evidence retained; final release open | The previously completed full rerun 3 passed reproducible images/SBOM/Trivy/runtime/lifecycle cleanup, and the published baseline clean clone passed `make check`. Remaining goals are the post-repair CI rerun, default native-expiry/native host evidence, live OpenAI opt-in, 4 GB performance, final docs/screenshots, and release decision |

Tasklist checkboxes remain stricter than this wave ledger. Only `M0-001`, `M0-002`, `M0-009`, `M0-015`, `M0-017`, and `M0-019` are complete. `M0-005` still lacks a live OpenAI result; the post-repair CI rerun, native host-nft, reusable worktree, Compose mutation, and five-minute 4 GB performance evidence remain incomplete. Therefore the current release classification is **Still implementing**.
