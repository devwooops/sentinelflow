...
| Leaf 1 | `compose_browser_qa` | Local-only Compose browser runner와 exact active/revoked action-state evidence | Fast browser QA가 sanitized active/revoked capture와 함께 exit `0`을 기록했다. Non-release UI evidence이며 final release certification/screenshot은 pending임 |
| Leaf 2 | `clean_checkout_gate` | Clean checkout과 CI reproducibility, publication authority 없음 | Commit `d66c4b8a4842ad4226cb741e35331ba5b9068520`를 clean temporary directory에 외부 clone했고 status는 전후 clean이며 `make check`를 통과했다. Hosted CI run `29696139988`은 `5ef870155bc59e6ac3c30279a7cd8be8d0249887`에서 10개 shard를 모두 통과했고 Linux 및 release gate는 open으로 유지함 |
| Leaf 3 | `expiry_bounds_v2_repair` | Executor result contract, dispatcher persistence, migration 34, bounded lifecycle diagnostic; frontend 제외 | Current-tree Linux native v6 E2E가 exit `0`을 기록했다. Bounded v2 TTL expiry, signed absence, audit/recovery/forwarding convergence 및 cleanup 뒤 unchanged semantic host nftables를 관측했고 release는 open으로 유지함 |
...
| --- | --- | --- |
| Contract and implementation wave | Published baseline, current local repair 및 hosted CI | Gateway-first contract, provider-free stale queued-analysis supersession·staged signed-history activation·repeated-content-digest identity·API-only projection을 포함한 publish된 33개 migration, backend/control plane, UI, dispatcher/executor, recovery, export, observability, test harness가 존재한다. Current uncommitted M34 repair는 `execution-result-v2` signed read-back bound, no reuse/no TTL refresh persistence, bounded diagnostic을 추가했고 focused test가 통과했다. Current-tree native v6 E2E도 real expiry, signed absence, recovery/forwarding/audit convergence 및 semantic host invariance로 exit `0`을 기록했다. `d66c4b8a4842ad4226cb741e35331ba5b9068520`은 이전 clean-clone/CI evidence이며 current-SHA CI는 아님 |
| Frontend contract certification | Final root 및 hosted CI 통과 | Final root rerun이 Vitest file 39개/test 363개와 deployment-CSP Chromium 1/1을 보고했다. CSP-safe error decoder, exact header parser, all-production-chunk dynamic-code-generation scan이 존재한다. 이후 macOS runner가 active/revoked browser QA를 통과했고 hosted CI run `29696139988`은 functional-browser와 pinned Linux visual-baseline gate를 통과했지만 full frontend state/accessibility 및 release-screenshot evidence는 pending임 |
| Backend and safety certification | Published final root rerun 통과; M34 focused check 통과 | Backend 88-package gate와 publish된 PostgreSQL 17.10 33-migration/72-table verifier의 final root rerun이 fresh/restart-noop·`33→24→33`·ACL·sqlc·repeated-content identity·API projection·stale-analysis supersession check를 포함해 통과했다. M34 v2 bounds/no-reuse database-chain과 focused contract/unit check도 통과했지만 native Linux와 release-duration condition은 open임 |
