...
| Leaf 3 | `docs_sync_current_evidence` | `README.md`와 PRD/ADR/TDD/Tasklist/WBS/readiness 영문·한글 pair | Running. Hosted-CI, browser-QA result, exact executor dependency policy 및 open release gate를 동기화하며 task completion은 변경하지 않음 |
...
| --- | --- | --- |
| Contract and implementation wave | Published baseline, local integration 및 hosted CI | Gateway-first contract, provider-free stale queued-analysis supersession·staged signed-history activation·repeated-content-digest identity·API-only projection을 포함한 33개 migration, backend/control plane, UI, dispatcher/executor, recovery, export, observability, test harness가 존재함. `d66c4b8a4842ad4226cb741e35331ba5b9068520`은 clean-clone `make check` evidence가 있고 hosted CI run `29696139988`은 `5ef870155bc59e6ac3c30279a7cd8be8d0249887`에서 10개 shard를 모두 통과함 |
...
| Supply chain and final release | 이전 full supply-chain, clean-clone 및 hosted-CI evidence 유지, final release open | 이전 full rerun 3은 reproducible image/SBOM/Trivy/runtime/lifecycle cleanup을 통과했고 publish된 baseline clean clone도 `make check`를 통과했으며 hosted CI run `29696139988`도 10개 shard를 모두 통과했다. 남은 목표는 default native-expiry/native host evidence, live OpenAI opt-in, 4 GB performance, final doc/screenshot 및 release decision임 |

Tasklist checkbox는 이 wave ledger보다 엄격하다. Complete 항목은 `M0-001`, `M0-002`, `M0-009`, `M0-015`, `M0-017`, `M0-019`뿐이다. Hosted CI는 `M0-006`과 `M0-008`의 독립 quality-gate deliverable을 제공하지만 unchecked `M0-003`과 `M0-007` prerequisite 때문에 Tasklist 항목은 open으로 유지한다. `M0-005`에는 여전히 live OpenAI result가 없고 native host-nft, reusable worktree, Compose mutation 및 five-minute 4 GB performance evidence가 미완료다. 따라서 현재 release classification은 **Still implementing**이다.
