...
3. Reproduce the complete mutation path from signed demo history through add, inspect, revoke, outage, restart, audit, and cleanup; RUN25 fast evidence covers the fast path, and current-tree native v6 evidence covers TTL expiry, signed absence, recovery, forwarding, audit, and host-ruleset invariance.
4. Preserve independent Linux evidence for native kernel expiry, host-ruleset invariance, and the five-minute 500-RPS/4-GB performance gate; final release remains separately gated.
...
The fixed adapter contract follows the official [`gpt-5.6-sol` model page](https://developers.openai.com/api/docs/models/gpt-5.6-sol), [model catalog](https://developers.openai.com/api/docs/models), and [Structured Outputs guide](https://developers.openai.com/api/docs/guides/structured-outputs). Any model or schema-version change requires contract revalidation and documentation updates. The opt-in `cmd/openaismoke` proves disabled and missing-key fail-closed behavior locally and has one verified, billable, non-mutating live result: `openai_responses` accepted `gpt-5.6-sol` for synthetic `path_scan` with one evidence reference; no persistence, HIL, dispatcher, or executor path was reached.
...
| FR-015 | MUST | Temporary rules must expire automatically and support safe read-only reconciliation plus separate manual revocation. | Native timeout removes the rule; new executor output signs `execution-result-v2` read-back start/completion times and positive remaining TTL, from which lifecycle retains lower/upper expiry bounds instead of inventing an exact timestamp. Separately signed `nft-inspect-v1` performs only fixed read-back and cannot mutate; a true premature disappearance is failed/alerted and never auto-readded, while boundary-overlapping or invalid evidence is indeterminate and fail closed; reapplication requires new validation/HIL; deterministic `nft-revoke-v1` can only delete from the owned set and is independently authorized, idempotently read back, and audited. |
...
| Reversibility | An approved temporary rule expires on schedule and is audited; expiry classification uses signed v2 read-back bounds, never an inferred single timestamp. |
...
Current evidence covers final root backend/data/contract/security/recovery/frontend gates, RUN25 fast Compose E2E, prior clean-clone/hosted-CI evidence, and a one-attempt billable live OpenAI probe that returned `openai_responses`/`gpt-5.6-sol` `status=ok` for synthetic `path_scan` with one evidence reference and no control-plane mutation. The current uncommitted implementation adds migration 34 and `execution-result-v2`: executor-signed read-back lower/upper expiry bounds, no result/bound reuse, no TTL refresh, and a bounded diagnostic projection. Its current-tree Linux native v6 E2E exited `0` and proved real kernel TTL expiry, signed absent inspection, audit/recovery/forwarding convergence, and unchanged semantic host nftables after cleanup. The current-tree five-minute 4 GB Linux performance gate exited `0` with `GATE_VERDICT=pass`, p95 `533us`, and outage overhead `436us`. Fast browser QA exited `0` with sanitized active/revoked screenshots but remains non-release UI evidence. This evidence still does not satisfy the release table: current-SHA committed clean-checkout/CI, final release screenshots/submission evidence, and the release decision remain pending. [Implementation Readiness](./IMPLEMENTATION_READINESS.md) records the detailed boundary.
